-- Hall of Fame v2: time window, the caller's own percentile, per-category and
-- per-table records, global card meta, tie handling, and supporting indexes.

create index if not exists idx_score_entries_points
  on score_entries (points desc) where points > 0;

create index if not exists idx_score_entries_category_points
  on score_entries (card_category, points desc) where points > 0;

create index if not exists idx_game_players_total_score
  on game_players (total_score desc);

create index if not exists idx_games_count_played_at
  on games (player_count, played_at desc);

-- Record holders on a tie, mirroring the local Records card: up to three names
-- plus the full holder count.
...
select jsonb_build_object(
  'names', coalesce(
    (select jsonb_agg(n order by n) from (select n from distinct_names order by n limit 3) t),
    '[]'::jsonb
  ),
...
-- Dropped rather than replaced: the added parameters would otherwise create an
-- overload, making the three-argument call from older clients ambiguous.
drop function if exists public.hall_of_fame(int, boolean, text);
...
select jsonb_build_object(
  ...
  'card_meta', jsonb_build_object(
    'most_played', (
      select jsonb_build_object('card_key', card_key, 'plays', plays)
      from card_plays order by plays desc, card_key limit 1
    ),
    'best_average', (
