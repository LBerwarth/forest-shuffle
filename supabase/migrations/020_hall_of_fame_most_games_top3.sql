-- Widens the most-active-player record to a top three. Clients shipped with
-- 019 read most_games as a single object, and installed PWAs keep serving that
-- bundle for a while, so it stays alongside the new most_games_top array.
