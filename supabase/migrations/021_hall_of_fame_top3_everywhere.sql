-- Widens every headline record to a top three, the way 020 did for most games.
-- Clients shipped before this read the singular keys, and installed PWAs keep
-- serving that bundle for a while, so those stay alongside the new arrays.
